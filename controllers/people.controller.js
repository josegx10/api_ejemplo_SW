...
export const getPeoples = async (req, res) => {
  ...
  if (req.query.search) {
    const cont = await People.find({name: new RegExp(req.query.search, 'i')}).count();
    let peoples = {
      count: cont,
      next:
        perPage * page < cont
          ? `http://localhost:4000/api/people/?search=${req.query.search}&page=${+page + 1}`
          : null,
      previous:
        page > 1 ? `http://localhost:4000/api/people/?search=${req.query.search}&page=${page - 1}` : null,
...
export const postPeople = async (req, res) => {
  ...
  const {name, heigth, mass, hair_color, skin_color, eye_color, birth_year, gender, homeworld, species, films, vehicles, starships} = req.body;
  const people = new People({name, heigth, mass, hair_color, skin_color, eye_color, birth_year, gender, homeworld, species, films, vehicles, starships});
  await people.save();
...
export const deletePeople = async (req, res) => {
  ...
  if (!deletedTask) return res.status(404).json({ message: "People not found" });
